...
class StatusCRUDTest(TestCase):
    ...
    def test_status_create(self):
        self.client.post(reverse('status_create'), {'name': 'New Status'})
        self.assertEqual(Status.objects.count(), 1)
    
    def test_status_update(self):
        status = Status.objects.create(name='Old Status')
        self.client.post(reverse('status_update', args=[status.pk]), {'name': 'Updated Status'})
        status.refresh_from_db()
        self.assertEqual(status.name, 'Updated Status')
    
    def test_status_delete(self):
        status = Status.objects.create(name='To Delete')
        self.client.post(reverse('status_delete', args=[status.pk]))
...
class LabelCRUDTest(TestCase):
    ...
    def test_label_create(self):
        self.client.post(reverse('label_create'), {'name': 'New Label'})
        self.assertEqual(Label.objects.count(), 1)
    
    def test_label_update(self):
        label = Label.objects.create(name='Old Label')
        self.client.post(reverse('label_update', args=[label.pk]), {'name': 'Updated Label'})
        label.refresh_from_db()
        self.assertEqual(label.name, 'Updated Label')
    
    def test_label_delete(self):
        label = Label.objects.create(name='To Delete')
        self.client.post(reverse('label_delete', args=[label.pk]))
